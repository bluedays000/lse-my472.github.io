---
title: "Introductory quantitative textual analysis"
...
```{r}
exemplary_corpus <- corpus(exemplary_documents,
                           docvars = data.frame(name = names(exemplary_documents),
                                                characters = str_count(exemplary_documents))
                           )
```
...
Based on this corpus, let us create a document-feature matrix (dfm). It has a row for each document and a column for each word/term. Cells contain word counts in the respective documents.

```{r}
dfmplain <- dfm(data_corpus_inaugural)
dfmplain
```
...
When building such a dfm, however, we might e.g. want to avoid counts of punctuation characters. For this we should make the tokenisation step explicit which allows us to specify certain options, here to remove punctuation (in fact the `dfm()` function without a token step will be deprecated):
...
```{r}
short_dictionary <- dictionary(list(taxation = c("tax", "taxes", "taxation"),
