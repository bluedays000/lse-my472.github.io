...
```{r}
authentication <- list(consumer_key = "0UWoK0DIw8ixMXvUlBczuVEU6",
                 consumer_secret = "TpYUabxYPfdksfWfuvbV4XaHIXfQdTfsgqQccULVF6930w3o3U",
                 access_token = "1451105644212367365-CXv3iuoklKG3doqvE7kxbcBfT1JQvv",
                 access_token_secret = "8Sbrxr91i1mg9DrwtWq2kmdwENUHipiJNAXMDx0nWGtne")
```
...
twitter_token <- create_token(app = "Week5 App", 
...
```{r}
keywords_tweets <- stream_tweets("pandemic", timeout = 5)
...
```{r}
stream_tweets(q = c(-125, 26, -65, 49), timeout = 30, parse = FALSE, file_name='geo_tweets2.json')
#stream_tweets(q = lookup_coords("usa"), timeout = 30, file_name='__________________')

geo_tweets <- tryCatch({parse_stream('test_json/streamed_geo_tweets.json')},
                       error = function(e)
                           {print(paste("Retrying with alternative function after initial error when parsing file",
                                        'test_json/streamed_geo_tweets.json'));
                           return(recover_stream('test_json/streamed_geo_tweets.json'))})
```
